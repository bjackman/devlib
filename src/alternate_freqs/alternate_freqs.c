...
int parse_int(const char *str, const char *err_fmt)
{
        char *endptr;
        int ret = strtol(str, &endptr, 0);
        if (endptr == str) {
                fprintf(stderr, err_fmt, str);
                exit(1);
        }
        return ret;
}
...
int main(int argc, char **argv)
{
        ...
        int cpu = parse_int(argv[1], "Invalid CPU \"%s\" (should be integer)");
        const char *freq1 = argv[2], *freq2 = argv[3];
        int interval_us = parse_int(argv[4], "Invalid interval \"%s\"");
        int num_loops = parse_int(argv[5], "Invalid loop count \"%s\"");
